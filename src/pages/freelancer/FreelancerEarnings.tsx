      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">${earningsData.currentBalance.toLocaleString()}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-yellow-600">${earningsData.pendingPayments.toLocaleString()}</div>
                    ...
                    <div key={transaction.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between text-sm text-gray-500">
                          <span>
                            {transaction.client ? transaction.client : transaction.method} • {new Date(transaction.date).toLocaleDateString()}
                          </span>
                  ...
                  {upcomingPayouts.map((payout) => (
                    <div key={payout.id} className="p-4 border rounded-lg">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="text-green-600 font-bold">${payout.amount.toLocaleString()}</span>
                      </div>
                      <p className="text-xs text-gray-500 mb-1">{payout.client}</p>
                      <p className="text-xs text-gray-600">Due: {new Date(payout.dueDate).toLocaleDateString()}</p>
                    </div>
                  ))}
                </div>
                
                <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                  <h4 className="font-semibold text-blue-900 text-sm mb-2">Withdrawal Information</h4>
